"use strict";(()=>{var e={};e.id=3001,e.ids=[3001,5748,6962],e.modules={399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},2048:e=>{e.exports=require("fs")},2615:e=>{e.exports=require("http")},8791:e=>{e.exports=require("https")},5315:e=>{e.exports=require("path")},8621:e=>{e.exports=require("punycode")},6162:e=>{e.exports=require("stream")},7360:e=>{e.exports=require("url")},1764:e=>{e.exports=require("util")},2623:e=>{e.exports=require("worker_threads")},1568:e=>{e.exports=require("zlib")},2254:e=>{e.exports=require("node:buffer")},6005:e=>{e.exports=require("node:crypto")},7561:e=>{e.exports=require("node:fs")},4492:e=>{e.exports=require("node:stream")},2477:e=>{e.exports=require("node:stream/web")},7261:e=>{e.exports=require("node:util")},2575:(e,t,s)=>{s.r(t),s.d(t,{originalPathname:()=>S,patchFetch:()=>k,requestAsyncStorage:()=>b,routeModule:()=>v,serverHooks:()=>I,staticGenerationAsyncStorage:()=>x});var o={};s.r(o),s.d(o,{GET:()=>y,POST:()=>w});var n=s(9303),a=s(8716),r=s(3131),i=s(7070),u=s(1585),c=s(6033),d=s(5748),l=s(3887),p=s(7745),m=s(1445),h=s(6403),g=s(8635);let f=u.Ry({content:u.Z_().min(1).max(1e4),stream:u.O7().optional()});async function w(e,{params:t}){try{let s=await (0,h.RA)(e);if(s instanceof i.NextResponse)return s;let{id:o}=await t,n=await e.json(),a=f.parse(n),[r]=await d.db.select().from(l.conversations).where((0,p.xD)((0,p.eq)(l.conversations.id,o),(0,p.eq)(l.conversations.organizationId,s.organizationId))).limit(1);if(!r)return i.NextResponse.json({error:{code:"NOT_FOUND",message:"Conversation not found"}},{status:404});let[u]=await d.db.select().from(l.businesses).where((0,p.eq)(l.businesses.id,s.businessId)).limit(1),[c]=await d.db.insert(l.messages).values({conversationId:o,businessId:s.businessId,organizationId:s.organizationId,role:"owner",contentType:"text",content:{text:a.content}}).returning(),w=(await d.db.select().from(l.messages).where((0,p.eq)(l.messages.conversationId,o)).orderBy((0,m.C)(l.messages.createdAt)).limit(10)).reverse().map(e=>`${e.role}: ${e.content?.text||""}`).join("\n"),y=`Previous conversation:
${w}

Owner's latest message: "${a.content}"

Respond helpfully as LocalGenius, their AI marketing assistant.`,v=u?{name:u.name,vertical:u.vertical,city:u.city,state:u.state}:void 0;if(a.stream||"text/event-stream"===e.headers.get("accept")){let e=new TextEncoder,t="",n=new ReadableStream({async start(n){try{for await(let s of(0,g.Vw)({prompt:y,businessContext:v}))t+=s,n.enqueue(e.encode(`data: ${JSON.stringify({chunk:s})}

`));let[a]=await d.db.insert(l.messages).values({conversationId:o,businessId:s.businessId,organizationId:s.organizationId,role:"assistant",contentType:"text",content:{text:t},aiModel:"claude-sonnet-4-20250514"}).returning();n.enqueue(e.encode(`data: ${JSON.stringify({message:{id:a.id,conversationId:o,role:"assistant",content:t,type:"text",createdAt:a.createdAt?.toISOString()}})}

`)),n.enqueue(e.encode("data: [DONE]\n\n")),n.close()}catch(t){n.enqueue(e.encode(`data: ${JSON.stringify({error:t instanceof Error?t.message:"Stream failed"})}

`)),n.close()}}});return new Response(n,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive"}})}let b=await (0,g.R_)({prompt:y,businessContext:v}),[x]=await d.db.insert(l.messages).values({conversationId:o,businessId:s.businessId,organizationId:s.organizationId,role:"assistant",contentType:"text",content:{text:b},aiModel:"claude-sonnet-4-20250514"}).returning();return i.NextResponse.json({data:{ownerMessage:c,assistantMessage:x},meta:{timestamp:new Date().toISOString()}},{status:201})}catch(t){if(t instanceof c.jm)return i.NextResponse.json({error:{code:"VALIDATION_ERROR",message:"Invalid message",details:t.errors}},{status:400});console.error("[messages] Error:",t);let e=t?.cause;return e&&console.error("[messages] Cause:",e.message),i.NextResponse.json({error:{code:"INTERNAL_ERROR",message:"Failed to send message"}},{status:500})}}async function y(e,{params:t}){let s=await (0,h.RA)(e);if(s instanceof i.NextResponse)return s;let{id:o}=await t,n=e.nextUrl.searchParams,a=n.get("before"),r=Math.min(parseInt(n.get("limit")||"50",10),100),u=[(0,p.eq)(l.messages.conversationId,o),(0,p.eq)(l.messages.organizationId,s.organizationId)];a&&u.push((0,p.lt)(l.messages.createdAt,new Date(a)));let c=await d.db.select().from(l.messages).where((0,p.xD)(...u)).orderBy((0,m.C)(l.messages.createdAt)).limit(r+1),g=c.length>r,f=g?c.slice(0,r):c;return i.NextResponse.json({data:{messages:f.reverse(),pagination:{hasMore:g,nextCursor:g?f[0]?.createdAt?.toISOString():null}},meta:{timestamp:new Date().toISOString()}})}let v=new n.AppRouteRouteModule({definition:{kind:a.x.APP_ROUTE,page:"/api/conversations/[id]/messages/route",pathname:"/api/conversations/[id]/messages",filename:"route",bundlePath:"app/api/conversations/[id]/messages/route"},resolvedPagePath:"/Users/sethshoultes/Local Sites/localgenius/src/app/api/conversations/[id]/messages/route.ts",nextConfigOutput:"",userland:o}),{requestAsyncStorage:b,staticGenerationAsyncStorage:x,serverHooks:I}=v,S="/api/conversations/[id]/messages/route";function k(){return(0,r.patchFetch)({serverHooks:I,staticGenerationAsyncStorage:x})}},6403:(e,t,s)=>{s.d(t,{RA:()=>i,WG:()=>c,yh:()=>u});var o=s(7070),n=s(6176),a=s(6091);let r=new TextEncoder().encode(process.env.JWT_SECRET||"dev-secret-change-in-production");async function i(e){let t=e.headers.get("authorization"),s=e.cookies.get("lg_session")?.value,a=t?.startsWith("Bearer ")?t.slice(7):s;if(!a)return o.NextResponse.json({error:{code:"UNAUTHORIZED",message:"Missing authentication"}},{status:401});try{let{payload:e}=await n._(a,r);if(!e.sub||!e.org||!e.biz)return o.NextResponse.json({error:{code:"INVALID_TOKEN",message:"Malformed token payload"}},{status:401});return{userId:e.sub,organizationId:e.org,businessId:e.biz,plan:e.plan||"base"}}catch{return o.NextResponse.json({error:{code:"TOKEN_EXPIRED",message:"Access token expired"}},{status:401})}}async function u(e){return new a.N({sub:e.userId,org:e.organizationId,biz:e.businessId,plan:e.plan}).setProtectedHeader({alg:"HS256"}).setIssuedAt().setExpirationTime("15m").sign(r)}async function c(e){return new a.N({sub:e,type:"refresh"}).setProtectedHeader({alg:"HS256"}).setIssuedAt().setExpirationTime("30d").sign(r)}},5748:(e,t,s)=>{s.d(t,{db:()=>c,getDb:()=>u});var o=s(2237),n=s(2787),a=s(3887);let r=null,i=null;function u(){return i||(i=(0,n.tS)(function(){if(!r){if(!process.env.DATABASE_URL)throw Error("DATABASE_URL environment variable is not set");r=(0,o.qn)(process.env.DATABASE_URL)}return r}(),{schema:a})),i}let c=new Proxy({},{get:(e,t)=>u()[t]})},8635:(e,t,s)=>{s.d(t,{L:()=>d,OB:()=>c,R_:()=>i,Vw:()=>u,f8:()=>l});var o=s(4588);let n=null;function a(){return n||(n=new o.default({apiKey:process.env.ANTHROPIC_API_KEY})),n}let r=`You are LocalGenius, an AI marketing assistant for local businesses. You speak like a capable, warm colleague — not a chatbot. You are the employee they always needed but could never afford.
...
Tone: warm, conversational, slightly proud of their business. Keep the entire digest under 350 words.`,model:"claude-haiku-4-5-20251001",maxTokens:768})}}};var t=require("../../../../../webpack-runtime.js");t.C(e);var s=e=>t(t.s=e),o=t.X(0,[9276,4149,9187,2787,5972,3883,4524,1585,4588,3737],()=>s(2575));module.exports=o})();